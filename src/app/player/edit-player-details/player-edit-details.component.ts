...
@Component({
  selector: 'app-player-edit-details',
  templateUrl: './player-edit-details.component.html',
  styleUrls: ['./player-edit-details.component.scss']
})
export class PlayerEditDetailsComponent implements OnInit {
  ...
  this.playerService.getPlayerDetails(this.params)
    .subscribe((player) => {
      ...
      if (player['data']) {
        ...
        this.dateOfBirth = new Date(this.player.dob);
...
onSubmit() {
  ...
  this.playerService.editUpdatePlayer(formInputData).pipe()
    .subscribe((res:any) => {
      this.formSubmitted = false;
      if (res) {
        this.toastr.success( res.message || 'Player Update Successfully.');
