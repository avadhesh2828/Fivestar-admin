...
export class AgentNewComponent implements OnInit {
  ...
  onSubmit() {
    this.submitted = true;
    this.f.checkadd.setValue(false);
    //enable business adress
    this.f.business_address.enable();
    this.f.business_country.enable();
    this.f.business_state.enable();
    ...
    if (this.newAgentForm.invalid || this.formError) {
      ...
      if (this.imgPath == '') {
        this.formError = "Agent Id Proof can't be empty.";
        this.formSubmitted = false;
        return;
      }
    const forminputdata = {
      ...
      business_address:this.f.business_address.value,
      business_country:this.f.business_country.value,
      business_state:this.f.business_state.value,
      business_city:this.f.business_city.value,
      business_pincode:this.f.business_pincode.value,
      kyc_type:this.f.kyc_type.value,
      kyc_status:1,
      status:1,
      agent_proof_image: this.imgURL,
      commission_type: this.f.commission_type.value,
      commission_amount: this.f.commission_amount.value,
    ...
    this.agentService.createAgent(forminputdata).pipe()
      .subscribe((res:any) => {
        ...
        let errorMessage = '';
        this.toastr.error(errorMessage || err.error.GlobalError || err.error.message || 'Some error occurred while creating new Agent.');
        this.formSubmitted = false;
      });
    }
  }
  ...
  handleReset() {
    ...
    this.f.agent_proof_image.setValue('');
    ...
    this.newAgentForm.controls['country'].setValue('160');
    this.newAgentForm.controls['state'].setValue('');
    this.newAgentForm.controls['business_country'].setValue('160');
    this.newAgentForm.controls['business_state'].setValue('');
    this.newAgentForm.controls['agent_proof_image'].setValue('');
    this.newAgentForm.controls['kyc_type'].setValue('');
    this.newAgentForm.controls['commission_type'].setValue('');
    this.newAgentForm.controls['commission_amount'].setValue('1');
    this.imgErrorMsg ='';
