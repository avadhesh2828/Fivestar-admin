...
export class EventsNewComponent implements OnInit {
  ...
  initContestForm() {
    ...
    this.contestForm = this.formBuilder.group({
      ...
      optMatches:['',[Validators.required]],
      optMatchesDates:[''],
    });
  }
  ...
  onSubmit() {
    ...
    if (this.contestForm.invalid || Object.keys(this.formError).length > 0) {
      ...
      this.eventService.createEvent({ event_details: formInputData }).pipe()
        .subscribe((res: any) => {
          this.formSubmitted = false;
          if (res.response_code == 200) {
            this.toastr.success(res.message || 'New Event Created Sucessfully.');
            this.formError = '';
