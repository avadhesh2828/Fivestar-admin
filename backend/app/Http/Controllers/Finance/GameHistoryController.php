...
class GameHistoryController extends Controller
{
    ...
    public function game_history( Request $request ){
        ...
        $gameHistory = $gameHistory->select('payment_history_transactions.*', 'U.username', DB::raw("(CASE WHEN payment_history_transactions.action = 'FiveStar Blessing' THEN payment_history_transactions.action ELSE NULL END) AS action"), DB::raw("(CASE WHEN payment_history_transactions.bet = 0 THEN payment_history_transactions.bet ELSE 'Free Spin' END) AS bet"));
